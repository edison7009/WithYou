...
全屏 Three.js 黄昏湖景：草岸、远山、水面反射、随风摆动的草、五位风格统一的 3D 演示人物与伙伴灯。
...
湖景标签页不可见时停止动画循环，恢复后继续。正常模式最高目标 40 FPS，省电模式为 24 FPS，并降低渲染分辨率与草密度；实际性能取决于设备，尚未建立集显性能基线。
...
| `lib/lake/shaders.ts` | 天空、夕阳和云层着色 |
